...
> boundary — a CDN can survive its origin dying only for content it already
> holds."
...
> "Four EC2 instances, four regions. The load balancer is my own FastAPI service
> — there's no AWS ALB in this account, because using one would have replaced
...
> fetches, the rest await the same in-flight result. Without it a cold-cache
...
> cap, so nothing ever evicts — with zero evictions the policies are
...
> auth on the admin endpoints. That's Phase 3 — Nginx with TLS termination and
...
> unaffected. Content no edge has cached returns 502 — there's nothing to serve.
...
> the load balancer it's about 95, and that's network-bound, not CPU-bound —
> adding concurrency past that only inflates latency, because the Atlantic round
